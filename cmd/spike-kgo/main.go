// spike-kgo 是 v0.4 Kafka 客户端选型 spike（twmb/franz-go, kgo）。
//
// 与 cmd/spike-sarama 严格同口径：
//   - 单 goroutine 异步喂 ~100 byte click_event JSON 30s
//   - 配置：lz4 / acks=leader / linger=5ms / max in-flight=5
//   - 30s 后停止生产，最多等 5s 让在飞 ack 落地
//   - 输出：RPS / latency p50 p99 / 总字节 / heap alloc 增量
//
// 不是生产代码 — Day 13 选型完成后会被删/归档。
package main

import (
	"context"
	"encoding/json"
	"flag"
	"fmt"
	"log"
	"runtime"
	"sort"
	"sync/atomic"
	"time"

	"github.com/twmb/franz-go/pkg/kgo"
)

const (
	topic    = "slink.click_events"
	duration = 30 * time.Second
	drainMax = 5 * time.Second
)

type payload struct {
	EventID string `json:"event_id"`
	Code    string `json:"code"`
	IP      string `json:"ip"`
	UA      string `json:"ua"`
	TS      int64  `json:"ts"`
}

func main() {
	brokers := flag.String("brokers", "localhost:19092", "kafka bootstrap (host)")
	flag.Parse()

	cli, err := kgo.NewClient(
		kgo.SeedBrokers(*brokers),
		kgo.DefaultProduceTopic(topic),
		// 关 idempotency 才允许 LeaderAck（kgo 默认 idempotency 强制 acks=all）；
		// 与 sarama 同口径对比，单 broker 没有 ISR，LeaderAck 足够。
		kgo.DisableIdempotentWrite(),
		kgo.RequiredAcks(kgo.LeaderAck()),
		kgo.ProducerBatchCompression(kgo.Lz4Compression()),
		kgo.ProducerLinger(5*time.Millisecond),
		kgo.MaxProduceRequestsInflightPerBroker(5),
		kgo.RecordDeliveryTimeout(10*time.Second),
	)
	if err != nil {
		log.Fatalf("new client: %v", err)
	}
	defer cli.Close()

	var (
		sent     atomic.Int64
		acked    atomic.Int64
		failed   atomic.Int64
		bytesOut atomic.Int64
		latCh    = make(chan time.Duration, 200_000)
	)

	runtime.GC()
	var memBefore runtime.MemStats
	runtime.ReadMemStats(&memBefore)

	ctx := context.Background()
	start := time.Now()
	deadline := start.Add(duration)

	for time.Now().Before(deadline) {
		body := payload{
			EventID: "00000000-0000-0000-0000-000000000000",
			Code:    code7(sent.Load()),
			IP:      "203.0.113.42",
			UA:      "Mozilla/5.0 (X11; Linux x86_64) Spike/1.0",
			TS:      time.Now().Unix(),
		}
		buf, _ := json.Marshal(&body)
		bytesOut.Add(int64(len(buf)))

		rec := &kgo.Record{
			Key:   []byte(body.Code),
			Value: buf,
		}
		enq := time.Now()
		cli.Produce(ctx, rec, func(_ *kgo.Record, err error) {
			if err != nil {
				failed.Add(1)
				return
			}
			acked.Add(1)
			select {
			case latCh <- time.Since(enq):
			default:
			}
		})
		sent.Add(1)
	}

	stopFeed := time.Now()
	feedDur := stopFeed.Sub(start)

	// 等 in-flight ack（最多 drainMax）
	flushCtx, cancel := context.WithTimeout(ctx, drainMax)
	defer cancel()
	if err := cli.Flush(flushCtx); err != nil {
		log.Printf("flush: %v", err)
	}

	runtime.GC()
	var memAfter runtime.MemStats
	runtime.ReadMemStats(&memAfter)

	close(latCh)
	lats := make([]time.Duration, 0, len(latCh))
	for d := range latCh {
		lats = append(lats, d)
	}
	sort.Slice(lats, func(i, j int) bool { return lats[i] < lats[j] })

	rps := float64(sent.Load()) / feedDur.Seconds()
	mbOut := float64(bytesOut.Load()) / 1024 / 1024
	heapDelta := int64(memAfter.HeapInuse) - int64(memBefore.HeapInuse)

	fmt.Println("=== spike-kgo ===")
	fmt.Printf("duration:    %s (feed) + drain (Flush)\n", feedDur.Truncate(time.Millisecond))
	fmt.Printf("sent:        %d\n", sent.Load())
	fmt.Printf("acked:       %d\n", acked.Load())
	fmt.Printf("failed:      %d\n", failed.Load())
	fmt.Printf("RPS (sent):  %.0f\n", rps)
	fmt.Printf("payload:     %.1f MB written\n", mbOut)
	if len(lats) > 0 {
		fmt.Printf("ack latency: p50=%s p90=%s p99=%s max=%s (n=%d)\n",
			lats[len(lats)*50/100],
			lats[len(lats)*90/100],
			lats[min(len(lats)*99/100, len(lats)-1)],
			lats[len(lats)-1],
			len(lats),
		)
	}
	fmt.Printf("heap delta:  %+d KB\n", heapDelta/1024)
	fmt.Printf("total alloc: %d KB / mallocs=%d\n",
		(memAfter.TotalAlloc-memBefore.TotalAlloc)/1024,
		memAfter.Mallocs-memBefore.Mallocs)
}

func code7(n int64) string {
	const alpha = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
	var b [7]byte
	for i := range b {
		b[i] = alpha[(n+int64(i)*7)%62]
	}
	return string(b[:])
}
